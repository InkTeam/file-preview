    ...
    private boolean killProcess() {
        ...
        try {
            if (props.getProperty("os.name").toLowerCase().contains("windows")) {
                ...
                if (s.contains("soffice.exe")) {
                    Runtime.getRuntime().exec("taskkill /im " + "soffice.exe" + " /f");
                    flag = true;
                }
            } else {
                Process p = Runtime.getRuntime().exec(new String[]{"sh","-c","ps -ef | grep " + "soffice"});
                ByteArrayOutputStream baos = new ByteArrayOutputStream();
                InputStream os = p.getInputStream();
                byte[] b = new byte[256];
                while (os.read(b) > 0) {
                ...
                String s = baos.toString();
                if (StringUtils.ordinalIndexOf(s, "soffice", 3) > 0) {
                    String[] cmd ={"sh","-c","kill -15 `ps -ef|grep " + "soffice" + "|awk 'NR==1{print $2}'`"};
